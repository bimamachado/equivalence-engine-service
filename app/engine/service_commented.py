...
# The `EquivalenceEngine` class is the high-level orchestrator for a single
# evaluation request. It coordinates:
#  - taxonomy lookup
#  - deterministic hard rules (fast early exit)
#  - mapping (text -> concept nodes) with caching and optional fallback
#  - vector construction and scoring
#  - decisioning and justification building
#  - optional evidence construction and audit persistence
#
# Keep this class focused on orchestration; heavy computations are delegated
# to modules under `app/engine/*` and `app/mapper`.
class EquivalenceEngine:
    ...
    def evaluate(self, req: EvaluateRequest, tenant_id: str) -> EvaluateResponse:
        """Evaluate a single request end-to-end.

        The method follows a clear sequence of steps (numbered in comments):
        1) taxonomy / validation
        2) hard rules (deterministic)
        3) mapping (cache + optional fallback)
        4) scoring (vector building + coverage)
        5) decision (policy -> accept/reject/review)
        6) justification (human-readable explanation)
        7) optional evidence assembly
        8) audit persistence

        The function is intentionally imperative so timings can be measured
        per-step (see `timer_ms()` usage) and to allow early return on hard
        rule failures.
        """
        ...
        with timer_ms() as t_total:
            # 1) Taxonomy lookup / basic validation
            #    - `pydantic` already validated input shapes; use this step for
            #      cross-field and taxonomy-specific checks. Here we load the
            ...
            #      mappers/LLMs (e.g. identical course codes, invalid carga, etc.)
            #    - if `blocked` is True we short-circuit and return.
            with timer_ms() as t:
            ...
            # 3) Mapping (cache-backed)
            #    - map textual ementas to taxonomy concepts via the configured
            #      `mapper` (primary) and optionally `fallback_mapper`.
            #    - results are cached by a hash of tenant+taxonomy+ementa to
            #      avoid repeated expensive calls.
            #    - `degraded` indicates whether the fallback mapper was used.
            degraded = False
            with timer_ms() as t:
                ...
                # if primary mapper returned nothing useful and policy allows,
                # try the fallback mapper to enter degraded mode.
            ...
            # 4) Scoring
            #    - convert mapped concepts into internal vectors with confidences
            #    - compute coverage, critical coverage and level penalties using
            #      taxonomy context (nodes)
            with timer_ms() as t:
            ...
            # 5) Decision
            #    - apply policy thresholds to the computed `score` and
            #      `cov_crit` to produce a final `decisao` and `motivo`.
            with timer_ms() as t:
                decisao, motivo = decide(req.policy, score, cov_crit, degraded_mode=degraded)
            timings.decide = t()

            # 6) Justification
            #    - build human-readable short and detailed explanations based on
            #      the decision, missing concepts, and scoring breakdown.
            with timer_ms() as t:
                curta, detalhada = build_justification(
            ...
            # 7) Evidence (optional)
            #    - if requested, assemble example covered concepts and missing
            #      concepts to help auditors and reviewers understand the result.
            evidence = None
